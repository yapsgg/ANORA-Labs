...
import { matchesShortcut, SHORTCUT_UNDO, SHORTCUT_REDO } from '@/lib/shortcuts';
...
export const useUndoRedo: UseUndoRedo = ({
  ...
  useEffect(() => {
    ...
    const keyDownHandler = (event: KeyboardEvent) => {
      // Check redo first since it's more specific (Cmd+Shift+Z)
      if (matchesShortcut(event, SHORTCUT_REDO)) {
        redo();
      } else if (matchesShortcut(event, SHORTCUT_UNDO)) {
        undo();
      }
    };
